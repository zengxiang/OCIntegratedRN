...
- (NSArray *) sliceImage: (UIImage *) image toLayersAlongAxis: (BCAxis) axis
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        CGImageRef cgImageRef = CGImageCreateWithImageInRect(image.CGImage, rect);
        UIImage *sliceImage = [UIImage imageWithCGImage:cgImageRef
                                                  scale:image.scale
...
- (void)setImaginaryLineWithColor:(UIColor *)borderColor cornerRadius:(CGFloat)cornerRadius {
    ...
    border.lineCap = kCALineCapRound;//@"square";
...
- (void)BezierPathImageViewToCornerWithCornerRadius:(CGFloat)radius {
